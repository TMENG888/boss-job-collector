...
$('startBtn').addEventListener('click', async () => {
  ...
  if ($('useCurrent').checked) {
    ...
    if (curPlatform === 'boss') {
      const unknown = cities.filter((c) => !/^\d{9}$/.test(c) && !CITY_MAP[c.replace(/市$/, '')]);
      if (unknown.length) note = '无法识别的城市：' + unknown.join('、') + '（已按全国处理；可改填9位城市代码）';
    }
  }
  setStatus('RUN', `正在启动（共 ${urls.length} 组搜索会话：城市×关键词）…`);
  chrome.runtime.sendMessage({ type: 'START', platform: curPlatform, urls, url: urls[0], target, enrich, note });
});
